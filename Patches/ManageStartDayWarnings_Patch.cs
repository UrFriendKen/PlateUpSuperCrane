...
{
    [HarmonyPatch]
    static class ManageStartDayWarnings_Patch
    {
        static readonly Type TARGET_TYPE = typeof(ManageStartDayWarnings);
        const bool IS_ORIGINAL_LAMBDA_BODY = false;
        const int LAMBDA_BODY_INDEX = 0;
        const string TARGET_METHOD_NAME = "OnUpdate";
        const string DESCRIPTION = "Fix Crane Start Day Warning when prep crane disabled"; // Logging purpose of patch
        ...
        {
            null,
            typeof(ManageStartDayWarnings).GetField("CranePlayers", BindingFlags.NonPublic | BindingFlags.Instance),
            typeof(EntityQuery).GetProperty("IsEmpty", BindingFlags.Public | BindingFlags.Instance).GetMethod,
            null,
            null
        };
        ...
        static readonly List<object> MODIFIED_OPERANDS = new List<object>()
        {
            ...
            null,
            null,
            typeof(ManageStartDayWarnings_Patch).GetMethod("ShouldWarn", BindingFlags.NonPublic | BindingFlags.Static),
        };

        static bool ShouldWarn(bool hasCranePlayers)
        {
            return !hasCranePlayers && Main.PrefManager.Get<bool>(Main.ALLOW_CRANE_DURING_PREP_ID);
        }
